...
def aantal_kluizen_vrij():
    ...
    aantal_vrije_kluizen = 12 - len(kluizen)
    print(f" - Er zijn: {aantal_vrije_kluizen} vrije kluizen.")
    return int(aantal_vrije_kluizen)
...
def nieuwe_kluis():
    ...
    vrijeKluizen = aantal_kluizen_vrij()
    infile = open('fa_testkluizen.txt', 'r')
    kluisGegevens = infile.readlines()

    for kluis in kluisGegevens:
        print(kluis)
        splittedlist = kluis.split(';')
        for kluisNummer in splittedlist:
            print(splittedlist)
            print(kluisNummer + "asdasdasdasdasdasdasdasdasas")
            if kluisNummer in kluisnummers:
                kluis_nummer = kluisNummer
    ...
    if (vrijeKluizen > 0):
        ...
        if (len(wachtwoord) >= 4 and (";" not in wachtwoord) and (kluis_nummer in kluisnummers)):
            save_kluis = open('kluizen.txt', 'a')
            save_kluis.write('{};{}\n'.format(kluis_nummer, wachtwoord))
            print("Kluisnummer: "+ kluis_nummer + " Wachtwoord: " + wachtwoord)
        return int(kluis_nummer)
    else:
        return -2
...
def kluis_teruggeven():
    ...
    for kluis in kluizen:
        ...
        if invoer_kluis == kluis:
            message = "Wachtwoord geaccepteerd."
            return True
        else:
